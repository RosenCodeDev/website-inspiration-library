...
const sizeContentViewport = async (page) => {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    await session.send('Browser.setWindowBounds', {
      ...
      bounds: {
        left: 0,
        top: 0,
        width: Math.round((bounds.width ?? metrics.outerWidth) + targetWidth - metrics.innerWidth),
        height: Math.round((bounds.height ?? metrics.outerHeight) + targetHeight - metrics.innerHeight),
        windowState: 'normal',
      },
    });
    await page.waitForTimeout(350);
    const current = await page.evaluate(() => ({ innerWidth, innerHeight }));
    if (current.innerWidth === targetWidth && current.innerHeight === targetHeight) break;
  }
  const finalMetrics = await page.evaluate(() => ({
    innerWidth, innerHeight, outerWidth, outerHeight, screenX, screenY, devicePixelRatio,
  }));
  if (finalMetrics.innerWidth !== targetWidth || finalMetrics.innerHeight !== targetHeight) {
    throw new Error('Viewport is ' + finalMetrics.innerWidth + 'x' + finalMetrics.innerHeight + ', expected 1440x900.');
  }
...
const encodeAndMakeContactSheet = async (recipe, masterPath) => {
  ...
  await runProcess(ffmpegPath, [
    ...
    '-vf', 'fps=0.5,scale=720:450,tile=2x2', '-frames:v', '1', contactPath,
  ], { stdio: ['ignore', 'ignore', 'pipe'] });
...
for (const recipe of selectedRecipes) {
  ...
  try {
    ...
    const metrics = await sizeContentViewport(page);
