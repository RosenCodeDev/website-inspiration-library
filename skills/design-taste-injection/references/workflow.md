...
Accept one reply containing any combination of `ACCEPT ALL`, `RNN ACCEPT`, `RNN SHOW ANOTHER CARD`, `RNN PIN THIS CARD`, `RNN DO NOT USE THIS CARD`, and `RNN SWAP <stable-id, exact display name, legacy exact title, source name, or canonical URL>`. Apply valid operations in order and leave only ambiguous slots unchanged. A replacement returns to pending review. Pins and exclusions do not imply acceptance. The legacy `SHOW ANOTHER SET` spelling is accepted only as a compatibility alias.
