...
A release candidate uses the subscription workflow: run `npm run prepare:inspiration-eval`, follow the generated runbook in a Codex task signed in with ChatGPT, then run `npm run verify:inspiration-eval -- --manifest <manifest.json>`. Inspect the rendered screenshots and create the fingerprint-bound approval with `npm run approve:inspiration-eval -- --report <report.json> --reviewer <name>`. Run `npm run release:check` before a release; it reruns `ci:check` and rejects a missing, stale, modified, or API-only approval. No API key is required. The former automated API evaluation remains an optional benchmark: set `RUN_API_INSPIRATION_EVAL=1`, then run `npm run test:inspiration-eval:api`. Its report cannot satisfy the release gate.
