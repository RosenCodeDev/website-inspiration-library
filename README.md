...
The workflow uses schema-v10 state with per-generation execution provenance, one context-free anchor per direction, a user-level seeded shuffle bag, and an ephemeral read-only Codex CLI task signed in with ChatGPT for default first-pass generation. The child receives one selected still and sealed payload through stdin, returns a strict text-file manifest, and never writes the preview itself; the coordinator materializes and validates it. This subscription path is context-limited, not filesystem-confidential or API-isolated. Initial category previews contain only a hero and opening module, and the coordinator validates H0 from the rendered DOM, computed styles, and slot pixels. After the user freezes one direction, parent Codex in the website project creates exactly three complete homepage variants, executes an eligible implementation path, and presents H0 plus four hero alternatives for image-led cards before adding the contract-constrained development tweak bar and one dense-page design gate. The sealed runner never handles those later stages. Typed lineage, three-variant/four-hero batch completeness, protected-layout fingerprints, tweak-bar production exclusion, and legacy resume checkpoints are state-validated. An explicitly selected stateless Responses API path remains available for high-assurance benchmarking. Clone Remix remains available for curated, measurable references with human approval; it is not a guarantee for arbitrary websites. `npm run test:public-clone` runs the network-dependent Aside pilot outside CI because public availability and bot protection are nondeterministic.
...
A release candidate uses one automatic subscription smoke evaluation: run `npm run test:inspiration-eval` on a machine actively authenticated with ChatGPT, then run `npm run release:check`. The evaluation produces one validated Spade H0, one built-in ImageGen asset, a complete homepage, a dense page, deterministic screenshots, a five-dimension automatic rubric, and a fingerprint-bound machine attestation. `release:check` reruns credential-free CI and rejects missing, stale, modified, API-only, or below-threshold evidence without generating content. No API key or named reviewer is required. `prepare:inspiration-eval` and `verify:inspiration-eval` remain available for inspecting deterministic packets and artifacts. The Responses API evaluation remains an explicit optional benchmark through `npm run test:inspiration-eval:api`; it cannot satisfy the release gate.

The current 34 source-identity inventories are labeled `codex-drafted`. They remain eligible as fingerprinted generation guardrails, but they are not independent human, legal, or QA review.
